#
# pc2hetero.py - top-level script that defines the overall GUi: tabs, etc. 
#
...
def write_config_file_cb(b):
    path_to_share = os.path.join('~', '.local','share','pc2hetero')
...
def get_config_files():
    ...
    if nanoHUB_flag:
        full_path = os.path.expanduser("~/data/results/.submit_cache/pc2hetero")  # does Windows like this?
    else:
        # local cache
        try:
            cachedir = os.environ['CACHEDIR']
            full_path = os.path.join(cachedir, "pc2hetero")
        except:
...
def run_done_func(s, rdir):
    ...
    if nanoHUB_flag:
        ...
        os.system("submit  mail2self -s 'nanoHUB pc2hetero' -t 'Your Run completed.'&")
...
def run_sim_func(s):
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 pc2hetero-r7 config.xml")   # "-r7" suffix??
        else:
